...
func (a assembleBranchWriter) assemblePointerNonSliceAnyValue() *Statement {
	return Case(Id(FieldPathIdentifier(*a.f))).Block(
		Id("ref").Dot(a.f.ValueTypeName).Call(Id(Title(a.f.ValueTypeName)+"ID").Call(Id("nextSeg").Dot("refID"))).Dot(a.f.ValueTypeName),
		...
		Switch(Id("nextSeg").Dot("kind")).Block(
			ForEachValueOfField(*a.f, func(valueType *ast.ConfigType) *Statement {
				...
				return Case(Id("ElementKind"+Title(a.v.Name))).Block(
					...
					Id("treeRef").Op(":=").Id("elementReference").Values(Dict{
						Id("OperationKind"):        Id("ref").Dot("OperationKind"),
						Id("ElementID"):            Id("nextSeg").Dot("id"),
						Id("ElementKind"):          Id("ElementKind" + Title(a.v.Name)),
						Id("ReferencedDataStatus"): Id("referencedDataStatus"),
						Id("ElementPath"):          Id("referencedElement").Dot("Path"),
					}),
...
func (a assembleBranchWriter) assemblePointerSliceNonAnyValue() *Statement {
	return Case(Id(FieldPathIdentifier(*a.f))).Block(
		...
		Id("treeRef").Op(":=").Id("elementReference").Values(Dict{
			Id("OperationKind"):        Id("ref").Dot("OperationKind"),
			Id("ElementID"):            Int().Call(Id("ref").Dot("ReferencedElementID")),
			Id("ElementKind"):          Id("ElementKind" + Title(a.f.ValueType().Name)),
			Id("ReferencedDataStatus"): Id("referencedDataStatus"),
			Id("ElementPath"):          Id("referencedElement").Dot("Path"),
		}),
		If(a.field().Op("==").Nil()).Block(
			a.field().Op("=").Make(Map(Id(a.f.ValueType().Name)).Id("elementReference")),
		),
...
func (a assembleBranchWriter) assemblePointerNonSliceNonAnyValue() *Statement {
	return Case(Id(FieldPathIdentifier(*a.f))).Block(
		...
		Id("referencedElement").Op(":=").Id("engine").Dot(Title(a.f.ValueType().Name)).Call(Id("ref").Dot("ReferencedElementID")).Dot(a.v.Name),
		Id("treeRef").Op(":=").Id("elementReference").Values(Dict{
			Id("OperationKind"):        Id("ref").Dot("OperationKind"),
			Id("ElementID"):            Int().Call(Id("ref").Dot("ReferencedElementID")),
			Id("ElementKind"):          Id("ElementKind" + Title(a.f.ValueType().Name)),
...
func (a assembleBranchWriter) assembleNonPointerNonSliceAnyValue() *Statement {
	return Case(Id(FieldPathIdentifier(*a.f))).Block(
		Switch(Id("nextSeg").Dot("id").Dot("kind")).Block(
			ForEachValueOfField(*a.f, func(valueType *ast.ConfigType) *Statement {
				return Case(Id("ElementKind"+Title(a.v.Name))).Block(
...
func (a assembleBranchWriter) assembleNonPointerNonSliceNonAnyValue() *Statement {
	return Case(Id(FieldPathIdentifier(*a.f))).Block(
		...
		Id("engine").Id("assemble"+Title(a.f.ValueType().Name)+"Path").Call(Id("child"), Id("p"), Id("pIndex").Op("+").Lit(1), Id("includedElements")),
		a.field().Op("=").Id("child"),
	)
}
