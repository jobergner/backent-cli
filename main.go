...
func writeToOutputFile(outputDecls []outputDeclaration, outputFileName, packageName, varPrefix string) {
	var buf bytes.Buffer
	...
	for _, outputDecl := range outputDecls {
		...
		if varPrefix == "" {
			...
			buf.WriteString("\n\nconst " + varPrefix + "_" + outputDecl.name + " string = `" + escapedValue + "`")
		}
	}
...
func main() {
	...
	varPrefix := flag.String("prefix", "", "variable prefix")
	flag.Parse()
	...
	writeToOutputFile(outputDecls, *outputFileName, *packageName, *varPrefix)
}
